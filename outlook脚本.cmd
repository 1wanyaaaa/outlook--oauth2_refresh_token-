...
def parse_account_line(line: str) -> tuple[str, str, str, str | None]:
    ...
    if len(parts) == 2:
        ...
        if not password:
            raise FetchError("\u5bc6\u7801\u4e0d\u80fd\u4e3a\u7a7a\u3002")
        return "password", email_addr, password, None
    if len(parts) != 4:
        raise FetchError("\u683c\u5f0f\u9519\u8bef\uff0c\u652f\u6301\uff1a\u90ae\u7bb1----\u5bc6\u7801  \u6216  \u90ae\u7bb1----\u5bc6\u7801/\u5360\u4f4d----client_id----refresh_token")
    email_addr, _unused_password, client_id, refresh_token = parts
    if not email_addr or "@" not in email_addr:
        raise FetchError("\u90ae\u7bb1\u683c\u5f0f\u770b\u8d77\u6765\u4e0d\u6b63\u786e\u3002")
    if not client_id:
    ...
    return "oauth", email_addr, client_id, refresh_token
...
def imap_password_login(email_addr: str, password: str) -> imaplib.IMAP4_SSL:
    context = ssl.create_default_context()
    imap = imaplib.IMAP4_SSL(IMAP_HOST, IMAP_PORT, ssl_context=context)
    try:
        ...
        try:
            ...
            pass
        raise FetchError(f"IMAP \u5bc6\u7801\u767b\u5f55\u5931\u8d25\uff1a{exc}") from exc
    return imap


def pop3_password_login(email_addr: str, password: str) -> poplib.POP3_SSL:
    ...
    try:
        ...
        try:
            ...
            pass
        raise FetchError(f"POP3 \u5bc6\u7801\u767b\u5f55\u5931\u8d25\uff1a{exc}") from exc
    return pop3
...
def fetch_imap_items(imap: imaplib.IMAP4_SSL, mailbox: str, limit: int, unseen_only: bool) -> list[MailItem]:
...
def fetch_pop3_items(pop3: poplib.POP3_SSL, limit: int) -> list[MailItem]:
    try:
        response, listings, _octets = pop3.list()
    except poplib.error_proto as exc:
        raise FetchError(f"POP3 \u5217\u51fa\u90ae\u4ef6\u5931\u8d25\uff1a{exc}") from exc

    message_numbers: list[int] = []
    for entry in listings:
        parts = entry.decode("ascii", "replace").split()
        if parts and parts[0].isdigit():
            message_numbers.append(int(parts[0]))
    selected = list(reversed(message_numbers))[: max(1, limit)]

    uid_map: dict[int, str] = {}
    try:
        _resp, uid_lines, _octets = pop3.uidl()
    ...
    items: list[MailItem] = []
    for idx, number in enumerate(selected, start=1):
        try:
            _resp, lines, _octets = pop3.retr(number)
        ...
        items.append(
            MailItem(
                index=idx,
                uid=uid_map.get(number, str(number)),
                sender=decode_mime(msg.get("From")),
                to=decode_mime(msg.get("To")),
                date=decode_mime(msg.get("Date")),
                subject=decode_mime(msg.get("Subject")),
...
def fetch_mail_items(conn: object, protocol: str, mailbox: str, limit: int, unseen_only: bool) -> list[MailItem]:
    if protocol == "pop3":
        return fetch_pop3_items(conn, limit)  # type: ignore[arg-type]
    return fetch_imap_items(conn, mailbox, limit, unseen_only)  # type: ignore[arg-type]
...
def run_menu(conn: object, protocol: str, email_addr: str, mailbox: str, limit: int, unseen_only: bool) -> None:
    output_dir = make_output_dir(email_addr)
    items = fetch_mail_items(conn, protocol, mailbox, limit, unseen_only)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("account_line", nargs="?", help="\u90ae\u7bb1----\u5bc6\u7801  \u6216  \u90ae\u7bb1----\u5bc6\u7801/\u5360\u4f4d----client_id----refresh_token")
    ...
    parser.add_argument("--protocol", choices=["auto", "imap", "pop3"], default="auto", help="\u5bc6\u7801\u6a21\u5f0f\u4f7f\u7528\u7684\u53d6\u4ef6\u534f\u8bae")
...
def connect_with_password(email_addr: str, password: str, protocol: str) -> tuple[object, str]:
    if protocol == "imap":
        print("\u6b63\u5728\u4f7f\u7528 IMAP \u5bc6\u7801\u767b\u5f55...")
        return imap_password_login(email_addr, password), "imap"
    if protocol == "pop3":
        print("\u6b63\u5728\u4f7f\u7528 POP3 \u5bc6\u7801\u767b\u5f55...")
        return pop3_password_login(email_addr, password), "pop3"
    ...
    try:
        print("IMAP \u5931\u8d25\uff0c\u6b63\u5728\u5c1d\u8bd5 POP3...")
        return pop3_password_login(email_addr, password), "pop3"
    except FetchError as exc:
        errors.append(str(exc))

    raise FetchError(
        "\u5bc6\u7801\u6a21\u5f0f\u767b\u5f55\u5931\u8d25\u3002Outlook \u53ef\u80fd\u7981\u7528\u4e86\u666e\u901a\u5bc6\u7801\u767b\u5f55\uff0c"
        "\u8bf7\u786e\u8ba4 IMAP/POP \u5df2\u5f00\u542f\uff0c\u6216\u4f7f\u7528\u5e94\u7528\u4e13\u7528\u5bc6\u7801\u3002\n"
        + "\n".join(errors)
    )


def close_connection(conn: object, protocol: str) -> None:
    try:
        if protocol == "pop3":
            conn.quit()  # type: ignore[attr-defined]
        else:
            conn.logout()  # type: ignore[attr-defined]
    except Exception:
        pass
...
def main() -> int:
    ...
    if not account_line:
        account_line = input("\u8bf7\u7c98\u8d34\uff1a\u90ae\u7bb1----\u5bc6\u7801  \u6216  \u90ae\u7bb1----\u5bc6\u7801/\u5360\u4f4d----client_id----refresh_token\n> ")
    try:
        auth_type, email_addr, secret, refresh_token = parse_account_line(account_line)
        if auth_type == "oauth":
            print("\u6b63\u5728\u6362\u53d6 access_token...")
            assert refresh_token is not None
            access_token, new_refresh_token = get_access_token(secret, refresh_token, args.tenant)
            ...
            conn: object = imap_login(email_addr, access_token)
            protocol = "imap"
        else:
            conn, protocol = connect_with_password(email_addr, secret, args.protocol)
            print(f"\u5df2\u8fde\u63a5\uff1a{protocol.upper()}")

        if args.no_menu:
            out_dir = make_output_dir(email_addr)
            items = fetch_mail_items(conn, protocol, args.mailbox, args.limit, args.unseen)
            count = print_batch(items, args.body, args.save, out_dir)
            print(f"\n\u5b8c\u6210\uff1a\u5df2\u62c9\u53d6 {count} \u5c01\u90ae\u4ef6\u3002")
            if args.save:
                print(f"\u4fdd\u5b58\u76ee\u5f55\uff1a{out_dir}")
        else:
            run_menu(conn, protocol, email_addr, args.mailbox, max(1, args.limit), args.unseen)
        close_connection(conn, protocol)
        return 0
